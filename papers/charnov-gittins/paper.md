...
\;>\;0. \tag{6}$$
Strictly positive everywhere. **Faster regrowth implies a higher giving-up density and a
shorter residence time.** The small-$r$ expansion is
...
residence times from an arrival at $x=1$), generated by `vault/_scripts/c25_whittle.py`:

| $r\tau$ | $\mathrm{GUD}(r)$ | $\Delta\mathrm{GUD}$ | $\mathrm{GUD}/\mathrm{GUD}_{\mathrm{MVT}}$ | $t^{*}(r)$ | $t^{*}/t^{*}_{\mathrm{MVT}}$ |
|---|---|---|---|---|---|
...
<!-- C25 §5 table, rows r·τ ∈ {0.05, 0.2, 1, 10}; MVT baseline GUD = 0.3000, t* = 1.204 -->
