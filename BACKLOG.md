...
**End of 2026-09-05.** The audit/kill phase is complete. Live registers: `vault/program.md` (items), `vault/walls.md` (23 named absences), `vault/method/failure-taxonomy.md` (26 modes). Papers: `papers/charnov-gittins/` postable after two referee rounds; `papers/audited-record/` parked as internal documentation until a comparator exists. **Direction for next sessions (owner, 2026-09-05):** derivation-first sessions on the bridges that held (C5/Gittins extensions: switching costs, uncertain quality, predation) aimed at falsifiable predictions; one bench build (P-088 flower array); no new scouts or meta-analyses unless asked. New method guard: `_scripts/refsweep.py` is a mandatory step before any computed note claims novelty.

**Rounds 8–9 (G34 deep inquiry + follow-ups), 2026-09-05:** G34 narrowed — same reserve recursion, different functionals; C33 numbers withdrawn after its positive control failed; C37 theorem with five failure boundaries and the ruin-theory parent uncited by both fields; C38 19-system margin table (movable setpoint sorts it); C39 blind governance re-code (direction confirmed, underpowered); C40 setpoint survival test (not falsified; p = 0.00014 with bats, 0.059 without; margin n = 4); C41 parent sweep (3/8 double rediscoveries — not a rule; third anchor is a diagnostic, proposal staged in `audits/staged/`). Toolkit: 4 new providers. **No release-grade finding yet.** Best remaining candidates: G36 soil T-value (quotable now, repackaged identity), C38/C40 setpoint mechanism (needs a survival compilation and hummingbird margins re-admitted under a stated rule), C5 preprint (unchanged, strongest).

**Round 6–7 (audit + fix + second sweep), 2026-09-05:** audits 06/07 found 7 hard numeric defects and the intersect.py blank-key bug; all fixed (C25 residence column ×3.8 and non-monotone; C29 0.30→0.218; C32 no identifiable β; C30 volcanic A=10.7, biotic row NARROWED via Jordan 2022; G32 narrowed to recovery-ecology-only, topology mediated). Four new gaps opened and computed: G34/C33 (bird reserve margin 31–57% vs grid 15–20%, adequacy bought on the demand side), G35/C34 (Haldane–Muller right to 1.6% for humans; α≈45 vs wafer 0.3–5), G36/C35 (soil Ha: T-value ≡ 1 by construction; conventional agriculture 0.011), G37/C36 (fishery management learning exponent 0.67–0.92 under US MSA vs ≥1 under weak governance). **Open:** re-check G27/G7/G25/G28 counts for the blank-key phantom (list in `audits/staged/recheck-list-FIX1.md`); scoped N_universe for every 2026-09-05 gap once OpenAlex resets; C25 §4 r→0 restatement (audit 06 item 9); C6 reciprocal edge to C35 done as a pointer only.
